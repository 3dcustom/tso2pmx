...
// 改変 --ここから--
    ...
    /// 表示するサブメッシュを指定するフラグ
    /// </summary>
    public List<bool> visible_meshes_flag;
    ...
    void _lineVertexBuffer_Created(object sender, EventArgs e)
    {
        ...
        using (GraphicsStream data = buffer.Lock(0, 0, LockFlags.None))
        {
            ...
            data.Write(new CustomVertex.PositionColored(60.0f, 0.0f, 0.0f, Color.Red.ToArgb()));
            data.Write(new CustomVertex.PositionColored(0.0f, 0.0f, 0.0f, Color.Green.ToArgb()));
            data.Write(new CustomVertex.PositionColored(0.0f, 60.0f, 0.0f, Color.Green.ToArgb()));
            data.Write(new CustomVertex.PositionColored(0.0f, 0.0f, 0.0f, Color.Blue.ToArgb()));
            data.Write(new CustomVertex.PositionColored(0.0f, 0.0f, 60.0f, Color.Blue.ToArgb()));

            for (int i = -10; i <= 10; i++)
            {
                data.Write(new CustomVertex.PositionColored(-50.0f, 0.0f, i * 5.0f, Color.Black.ToArgb()));
                if (i == 0) data.Write(new CustomVertex.PositionColored(0.0f, 0.0f, i * 5.0f, Color.Black.ToArgb()));
                else data.Write(new CustomVertex.PositionColored(50.0f, 0.0f, i * 5.0f, Color.Black.ToArgb()));
            }
            for (int i = -10; i <= 10; i++)
            {
                data.Write(new CustomVertex.PositionColored(i * 5.0f, 0.0f, -50.0f, Color.Black.ToArgb()));
                if (i == 0) data.Write(new CustomVertex.PositionColored(i * 5.0f, 0.0f, 0.0f, Color.Black.ToArgb()));
    ...
    private void DrawLine()
    {
        ...
        device.SetStreamSource(0, _lineVertexBuffer, 0);
        device.VertexFormat = CustomVertex.PositionColored.Format;
        device.DrawPrimitives(PrimitiveType.LineList, 0, (3 + 21 + 21));
