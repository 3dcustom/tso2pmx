...
// 改変 --ここから--
        ...
        /// </summary>
        /// <param name="angle">角度（ラジアン）</param>
        public void RotateX2(float angle)
        {
            if (angle == 0.0f)
                return;
            ...
            // 移動を保存し、行列から移動を消去する
            Vector3 tmp_translation = new Vector3(m.M41, m.M42, m.M43);
            m.M41 = 0.0f;
            m.M42 = 0.0f;
            m.M43 = 0.0f;

            m *= Matrix.RotationQuaternion(qt); // 移動を含めずに回転させてから、
            m *= Matrix.Translation(tmp_translation); // 移動の情報を戻す
        }
        ...
        /// </summary>
        /// <param name="angle">角度（ラジアン）</param>
        public void RotateY2(float angle)
        {
            if (angle == 0.0f)
                return;
        ...
        /// </summary>
        /// <param name="angle">角度（ラジアン）</param>
        public void RotateZ2(float angle)
        {
            if (angle == 0.0f)
                return;
